...
const DEFAULT_COORDS = { lat: 41.7167, lon: 44.7833 }; // Tbilisi
...
export class WeatherService {
  ...
  _fetchAndSchedule() {
    this.fetchWeather()
      ...
      .catch(() => {
        // keep last cache; notify but don't spam
        if (this.cache) this.onUpdate(this.cache);
      })
      .finally(() => {
        const delay = this.options.refreshMinutes * 60 * 1000;
        const backoff = exponentialBackoff(this.attempt, 2000, 15 * 60 * 1000);
